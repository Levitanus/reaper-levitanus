...
use rea_rs::{
    ActionHook, ControlSurface, ExtState, HasExtState, Mutable, Project, Reaper, Timer, Track,
    TrackSend, UndoFlags,
};
...
fn move_track_before(track: &mut CachedTrack, before_index: usize) -> anyhow::Result<()> {
    ...
    let moved = Reaper::get().low().ReorderSelectedTracks(before_index as i32, 0);
    if !moved {
...
fn align_all_instrument_track_orders(
