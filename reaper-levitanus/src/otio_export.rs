...
pub fn export_otio_project() -> Result<(), anyhow::Error> {
    ...
    for plan in plans {
        ...
        let otio_rate =
            detect_otio_rate(&pr, &video_slices, fps_policy).unwrap_or(DEFAULT_OTIO_RATE);
...
fn collect_video_slices(pr: &Project, plan: &RenderTargetPlan) -> anyhow::Result<Vec<VideoSlice>> {
    ...
    for track_idx in tracks {
        ...
        for item_idx in 0..track.n_items()? {
            ...
            let clipped_start = if item_start > bound_start {
                item_start
            } else {
                bound_start
            };
            let clipped_end = if item_end < bound_end {
                item_end
            } else {
                bound_end
            };
...
fn apply_crossfade_trim(slices: &mut Vec<VideoSlice>) {
    ...
    slices.retain(|s| s.timeline_end > s.timeline_start && s.source_end > s.source_start);
}
...
fn slice_to_otio_clip(slice: &VideoSlice, otio_rate: f64) -> anyhow::Result<Clip> {
    ...
    let use_timewarp =
        (time_scalar - 1.0).abs() > PLAY_RATE_EFFECT_EPSILON && !KDENLIVE_COMPAT_DISABLE_TIMEWARP;
    let serialized_duration = if use_timewarp {
        source_duration
    } else {
        timeline_duration
    };
    let min_clip_duration = if otio_rate > 0.0 {
        1.0 / otio_rate
    } else {
        0.0
    };
    ...
    let unique_name = format!(
        "{} - t{:.3}-s{:.3}",
        base_name, slice.timeline_start, slice.source_start
    );
...
fn make_audio_clip(plan: &RenderTargetPlan, otio_rate: f64) -> anyhow::Result<Option<Clip>> {
    ...
    let duration = (plan.bound.end - plan.bound.start)
        .as_duration()
        .as_secs_f64();
    if duration <= 0.0 {
        return Ok(None);
...
fn detect_otio_rate(pr: &Project, slices: &[VideoSlice], policy: OtioFpsPolicy) -> Option<f64> {
    let files = slices
        .iter()
        .map(|s| s.file.clone())
        .collect::<HashSet<_>>()
                        ...
                        .and_then(Value::as_object_mut)
                    {
                        ...
                        if is_external_ref && !has_available_range {
                            if let Some(target_url) =
                                default_media.get("target_url").and_then(Value::as_str)
                            {
